    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-lg p-6">
                ...
                <form action="{{ route('box.update', $box->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="address" name="address" value="{{ $box->address }}" 
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:outline-none">
                        </div>
                        <div>
                            <label for="code" class="block text-gray-700 font-semibold mb-1">Code postal :</label>
                            <input type="text" id="code" name="code" value="{{ $box->code }}" 
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:outline-none">
                        </div>
                        <div>
                            <label for="city" class="block text-gray-700 font-semibold mb-1">Ville :</label>
                            <input type="text" id="city" name="city" value="{{ $box->city }}" 
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:outline-none">
                        </div>
                        <div>
